...
class StudentPrivatesTable extends React.Component {
  ...
  render() {
    let privates = _.get(this.props, 'StudentPrivates.privates') || null;
    privates = this.formatData(privates);
    
    return (
      <div className="table-wrapper">
        <div className="row table-filter-container">
          <TableFilter table={'private'} StudentPrivates={_.get(this.props, 'StudentPrivates.privates')} onChange={this.logChange.bind(this)} />
        </div>
        {
            privates.length ?
              <DataTable
